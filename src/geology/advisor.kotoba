(ns geology.advisor
  "SurveyAdvisor — proposes a survey operation (analyze survey data,
  draft a geological report, flag an anomalous formation, request field
  equipment) for a registered survey site. The advisor is swappable:
  ...
  and has no notion of site provenance or geological hazard risk;
  `geology.governor` is the independent system that decides whether the
  proposal may proceed, per the itonami actor pattern.

  A proposal is a map:
    {:op :analyze-survey-data|:draft-report|:flag-anomalous-formation
...
(def ^:private system-prompt
  "You are a geological survey advisor for geophysics and geological
   survey operations. Given a survey operation request, propose an :op,
  ...
  (reify Advisor
    (-advise [_ _store request]
      (let [msgs [{:role :system :content system-prompt}
                  {:role :user :content (str "survey operation request: " (pr-str request))}]
