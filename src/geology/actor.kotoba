(ns geology.actor
  "SurveyActor — the ISCO-08 2114 survey support actor as a
  `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one survey operation request
  ...
  The unconditional invariant: the SurveyAdvisor can never
  directly commit a record or dispatch a robot action the
  SurveyGovernor refuses — every commit-record! call is gated
  behind `:decide`."
...
(defn build-graph
  "Build a compiled SurveyActor graph. `store` implements
  `geology.store/Store`. `advisor` implements
  `geology.advisor/Advisor` (defaults to `mock-advisor`).
...
(defn run-request!
  "Run one survey operation request to completion or interrupt.
